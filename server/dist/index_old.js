...
function inferSports(text) {
    ...
    return Object.keys(sportKeywords).filter((sport) => sportKeywords[sport].some((keyword) => normalized.includes(keyword)));
}
...
app.post('/api/profiles', (req, res) => {
    ...
    const profile = { ...parsed.data, id: randomUUID(), availability: 'maybe', achievements: ['New teammate'] };
    users.push(profile);
...
app.post('/api/groups/:id/messages', (req, res) => {
    ...
    if (!parsed.success)
        return res.status(400).json({ error: parsed.error.flatten() });
    const message = { id: randomUUID(), groupId: req.params.id, userId: parsed.data.userId, body: parsed.data.body, createdAt: new Date().toISOString() };
...
app.post('/api/events', (req, res) => {
    ...
    if (!parsed.success)
        return res.status(400).json({ error: parsed.error.flatten() });
    const event = { ...parsed.data, id: randomUUID(), participantIds: Array.from(new Set([parsed.data.ownerId, ...parsed.data.participantIds])) };
    events.push(event);
