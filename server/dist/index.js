...
const sportValues = ['Football', 'Tennis', 'Basketball', 'Running', 'Volleyball', 'Padel'];
const skillValues = ['Beginner', 'Intermediate', 'Advanced'];
const availabilityValues = ['yes', 'no', 'maybe'];
const groupSize = {
...
function pickGroupName(sport, usedNames) {
...
let users = initialDatabase.users.length ? initialDatabase.users : seedUsers;
let groups = initialDatabase.groups;
let messages = initialDatabase.messages;
let events = initialDatabase.events;
function persist() {
    writeDatabase({
        users,
        groups,
...
function compatibility(memberIds, sport) {
    const members = memberIds
        .map((id) => users.find((user) => user.id === id))
        .filter(Boolean);
    const availableScore = members.filter((user) => user.availability === 'yes').length / Math.max(members.length, 1);
    const sportScore = members.filter((user) => user.sports.includes(sport)).length / Math.max(members.length, 1);
    const centroid = {
    ...
    const avgDistance = members.reduce((sum, user) => sum + distanceKm(user, centroid), 0) /
...
function selectCaptain(memberIds) {
    const sorted = [...memberIds].sort((a, b) => {
        const userA = users.find((user) => user.id === a);
        const userB = users.find((user) => user.id === b);
        const scoreA = (userA.achievements.includes('Captain material') ? 2 : 0) +
            (userA.skill === 'Advanced' ? 1 : 0);
        const scoreB = (userB.achievements.includes('Captain material') ? 2 : 0) +
...
function generateFreshGroups() {
    ...
    const available = users.filter((user) => user.availability === 'yes' || user.availability === 'maybe');
    Object.keys(groupSize).forEach((sport) => {
...
function inferSports(text) {
    const normalized = text
        .toLowerCase()
        .replace(/[^a-z0-9\s-]/g, ' ')
        .replace(/\s+/g, ' ')
        .trim();
    const scores = Object.keys(sportKeywords).map((sport) => {
        const score = sportKeywords[sport].reduce((total, keyword) => {
            const normalizedKeyword = keyword.toLowerCase();
            if (!normalized.includes(normalizedKeyword))
                return total;
            if (normalizedKeyword === sport.toLowerCase())
                return total + 5;
...
app.post('/api/profiles', (req, res) => {
    ...
    const emailExists = users.some((user) => user.email.toLowerCase() === parsed.data.email.toLowerCase());
    if (emailExists) {
...
app.post('/api/ai/infer-sports', (req, res) => {
    ...
    const inferredSports = inferSports(`${parsed.data.description} ${parsed.data.photoFileName ?? ''}`);
    res.json({
